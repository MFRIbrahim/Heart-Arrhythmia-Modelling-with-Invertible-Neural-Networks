...
def create_propagation_matrix(block_pattern, total_signals, offset=0):
    ...
    av_length = sum(block_pattern)
    av_constraints = create_pattern_level(
        atrial_constraints, block_pattern, av_length, level=1)

    v_constraints = create_regular_2_to_1_level(
        av_constraints, total_signals=total_signals, level=2)

    return v_constraints[offset:]
