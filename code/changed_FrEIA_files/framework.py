...
class ReversibleGraphNet(nn.Module):
    ...
    def forward(self, x, c=None, rev=False, intermediate_outputs=False, recurrent=False):
        ...
        for o in use_list:
            try:
                ...
                if self.module_cond[o[0]]:
                    ...
                    results = self.module_list[o[0]](x, c=c, rev=rev, recurrent=recurrent)
                else:
                    results = self.module_list[o[0]](x, rev=rev, recurrent=recurrent)
            except TypeError:
