...
class GLOWCouplingBlock(nn.Module):
    ...
    def forward(self, x, c=[], rev=False, recurrent=False):
        x1, x2 = (x[0].narrow(1, 0, self.split_len1),
                  x[0].narrow(1, self.split_len1, self.split_len2))
        if not rev:
            if recurrent:
                x2_cond = torch.cat([x2, *c], 1).permute(0,2,1)
                x2 = x2.permute(0,2,1)
                r2 = self.s2(x2_cond if self.conditional else x2)
                x2 = x2.permute(0,2,1)
                r2 = r2.permute(0,2,1)
                s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
                y1 = self.e(s2) * x1 + t2
                y1_cond = torch.cat([y1, *c], 1).permute(0,2,1)
                y1 = y1.permute(0,2,1)
                r1 = self.s1(y1_cond if self.conditional else y1)
                y1 = y1.permute(0,2,1)
                r1 = r1.permute(0,2,1)
            ...
            if recurrent:
                x1_cond = torch.cat([x1, *c], 1).permute(0,2,1)
                x1 = x1.permute(0,2,1)
                r1 = self.s1(x1_cond if self.conditional else x1)
                x1 = x1.permute(0,2,1)
                r1 = r1.permute(0,2,1)
                s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
                y2 = (x2 - t1) / self.e(s1)
                y2_cond = torch.cat([y2, *c], 1).permute(0,2,1)
                y2 = y2.permute(0,2,1)
                r2 = self.s2(y2_cond if self.conditional else y2)
                y2 = y2.permute(0,2,1)
                r2 = r2.permute(0,2,1)
                s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
                y1 = (x1 - t2) / self.e(s2)
