...
def train_splitter(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_cINN_splitter3(config.batch_size * config.n_iterations)
...
def train_signals_recurrent_matching(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.get_signals_recurrent_matching_batch(config.batch_size * config.n_iterations)
    ...
    for i in range(len(x)):
        if x[i].shape[0] >= config.batch_size:
            ...
            for j in range(len(x[i])//config.batch_size):
                ...
                log_jac = model.log_jacobian(run_forward=False)
                loss = torch.mean(torch.sum(0.5 * torch.sum(z_i**2, dim=1) - log_jac, dim=1))
                l_tot += (config.batch_size / (config.batch_size * config.n_iterations)) * loss.data.item()
                loss.backward()
                optim.step()
            ...
            if (len(x[i])//config.batch_size) * config.batch_size != len(x[i]):
                ...
                log_jac = model.log_jacobian(run_forward=False)
                loss = torch.mean(torch.sum(0.5 * torch.sum(z_i**2, dim=1) - log_jac, dim=1))
                l_tot += ((len(x[i]) - beg) / (config.batch_size * config.n_iterations)) * loss.data.item()
                loss.backward()
                optim.step()
            ...
            log_jac = model.log_jacobian(run_forward=False)
            loss = torch.mean(torch.sum(0.5 * torch.sum(z_i**2, dim=1) - log_jac, dim=1))
            l_tot += (len(x[i]) / (config.batch_size * config.n_iterations)) * loss.data.item()
            loss.backward()
            optim.step()
...
def train_signals_recurrent(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.get_signals_recurrent_batch(config.batch_size * config.n_iterations)
...
def train_signals_matching(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.get_signals_matching_batch(config.batch_size * config.n_iterations)
    for i in range(0, len(x), config.batch_size):
        x_i = x[i:i+config.batch_size].cuda()
        y_i = y[i:i+config.batch_size].cuda()
        optim.zero_grad()
...
def train_signals_sequence(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.get_signals_sequence_batch(config.batch_size * config.n_iterations)
    for i in range(0, len(x), config.batch_size):
        x_i = x[i:i+config.batch_size].cuda()
        y_i = y[i:i+config.batch_size].cuda()
        optim.zero_grad()
...
def train_matching_old(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_matching_batch_old(config.batch_size * config.n_iterations)
    for i in range(0, len(x), config.batch_size):
        x_i = x[i:i+config.batch_size].cuda()
        y_i = y[i:i+config.batch_size].cuda()
        optim.zero_grad()
...
def train_seq_old(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_seq_batch_old(config.batch_size * config.n_iterations)
    for i in range(0, len(x), config.batch_size):
        x_i = x[i:i+config.batch_size].cuda()
        y_i = y[i:i+config.batch_size].cuda()
        optim.zero_grad()
...
def train_cINN_old(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_cINN_batch_old(config.batch_size * config.n_iterations)
    for i in range(0, len(x), config.batch_size):
        x_i = x[i:i+config.batch_size].cuda()
        y_i = y[i:i+config.batch_size].cuda()
        optim.zero_grad()
...
def train_rcINN_matching_old(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_rcINN_matching_batch_old(config.batch_size * config.n_iterations)
...
def train_rcINN_old(model, optim):
    model.train()
    l_tot = 0
    x, y = datagen.generate_rcINN_batch_old(config.batch_size * config.n_iterations)
...
def make_stats(filtered_bp, true_stats, name, signals=False, splitter=False):
    ...
    for i in range(len(filtered_bp)):
        for j in range(len(filtered_bp[i][0])):  
            if filtered_bp[i][1][j] == block_type:
                if signals:
                    same_bp.append([[filtered_bp[i][0][j]], [filtered_bp[i][1][j]], filtered_bp[i][2], filtered_bp[i][3]])
                else:
                    same_bp.append([[filtered_bp[i][0][j]], [filtered_bp[i][1][j]], filtered_bp[i][2]])
            else:
                if signals:
                    alt_bp.append([[filtered_bp[i][0][j]], [filtered_bp[i][1][j]], filtered_bp[i][2], filtered_bp[i][3]])
                else:
                    alt_bp.append([[filtered_bp[i][0][j]], [filtered_bp[i][1][j]], filtered_bp[i][2]])
    ...
    if not splitter:
        for i in range(len(same_bp)):
            ...
            for j in range(len(same_bp[i][0])):
                if same_bp[i][1][j] == "1":
                    itv = simulate_type_1(same_bp[i][0][j][1], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2a":
                    itv = simulate_type_2a(same_bp[i][0][j][1], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2b":
                    itv = simulate_type_2b(same_bp[i][0][j][1], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2c":
                    itv = simulate_type_2c(same_bp[i][0][j][0], same_bp[i][0][j][1], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "3":
                    itv = simulate_type_3(same_bp[i][0][j][0], same_bp[i][0][j][1], same_bp[i][0][j][2], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
        ...
        for i in range(len(alt_bp)):
            ...
            for j in range(len(alt_bp[i][0])):
                if alt_bp[i][1][j] == "1":
                    itv = simulate_type_1(alt_bp[i][0][j][1], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2a":
                    itv = simulate_type_2a(alt_bp[i][0][j][1], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2b":
                    itv = simulate_type_2b(alt_bp[i][0][j][1], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2c":
                    itv = simulate_type_2c(alt_bp[i][0][j][0], alt_bp[i][0][j][1], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "3":
                    itv = simulate_type_3(alt_bp[i][0][j][0], alt_bp[i][0][j][1], alt_bp[i][0][j][2], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
    ...
    if splitter: 
        for i in range(len(same_bp)):
            ...
            for j in range(len(same_bp[i][0])):
                if same_bp[i][1][j] == "1":
                    itv = simulate_type_1(same_bp[i][0][j][0], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2a":
                    itv = simulate_type_2a(same_bp[i][0][j][0], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2b":
                    itv = simulate_type_2b(same_bp[i][0][j][0], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if same_bp[i][1][j] == "2c":
                    itv = simulate_type_2c(same_bp[i][0][j][0], same_bp[i][0][j][1], same_bp[i][2][0], same_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
        ...
        for i in range(len(alt_bp)):
            ...
            for j in range(len(alt_bp[i][0])):
                if alt_bp[i][1][j] == "1":
                    itv = simulate_type_1(alt_bp[i][0][j][0], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2a":
                    itv = simulate_type_2a(alt_bp[i][0][j][0], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2b":
                    itv = simulate_type_2b(alt_bp[i][0][j][0], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
                if alt_bp[i][1][j] == "2c":
                    itv = simulate_type_2c(alt_bp[i][0][j][0], alt_bp[i][0][j][1], alt_bp[i][2][0], alt_bp[i][2][1])
                    itv_sub.append(itv[:(n_Rwaves-1)])
    ...
    for i in range(len(intervals_same)):
        for j in range(len(intervals_same[i])):
            differences_same.append(np.mean(np.abs(intervals_same[i][j]-y_true[:(n_Rwaves-1)])))
            indeces_same.append([i,j])
    
    for i in range(len(intervals_alt)):
        for j in range(len(intervals_alt[i])):
            differences_alt.append(np.mean(np.abs(intervals_alt[i][j]-y_true[:(n_Rwaves-1)])))
    ...
    if len(differences_alt) + len(differences_same) > 0:
        alt_same_ratio = len(differences_alt) / (len(differences_alt) + len(differences_same))
    ...
    if len(differences_same) != 0:
        append_new_line(f'TEST_top1_diff_same_{name}.txt', str(differences_same[0]))
    
    if len(differences_alt) != 0:
        append_new_line(f'TEST_top1_diff_alt_{name}.txt', str(differences_alt[0]))
    ...
    for i in range(len(differences_same[:5])):
        ...
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(intervals_same[indeces_same[i][0]][indeces_same[i][1]]))
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(differences_same[i]))
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(same_bp[indeces_same[i][0]][0][indeces_same[i][1]]))
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(same_bp[indeces_same[i][0]][1][indeces_same[i][1]]))
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(same_bp[indeces_same[i][0]][2]))
        if signals:
            append_new_line(f'TEST_top10_sol_same_{name}.txt', str(same_bp[indeces_same[i][0]][3]))
        append_new_line(f'TEST_top10_sol_same_{name}.txt', "--------------------------")

    
    for i in range(len(differences_alt[:5])):
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', "--------------------------")
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(intervals_alt[indeces_alt[i][0]][indeces_alt[i][1]]))
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(differences_alt[i]))
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(alt_bp[indeces_alt[i][0]][0][indeces_alt[i][1]]))
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(alt_bp[indeces_alt[i][0]][1][indeces_alt[i][1]]))
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(alt_bp[indeces_alt[i][0]][2]))
        if signals:
            append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(alt_bp[indeces_alt[i][0]][3]))
        append_new_line(f'TEST_top10_sol_alt_{name}.txt', "--------------------------")

    
    if signals:
        block_pattern_s = datagen.correct_bp(block_pattern_true, block_type, n_Rwaves)
        signals_true = np.array(datagen.bp_to_signals(block_pattern_s, block_type, n_Rwaves, fill=False), dtype='int16')
    append_new_line(f'TEST_top10_sol_same_{name}.txt', "=====================================================")
    append_new_line(f'TEST_top10_sol_same_{name}.txt', str(y_true[:(n_Rwaves-1)]))
    ...
    append_new_line(f'TEST_top10_sol_same_{name}.txt', str(atrial_cycle_length))
    append_new_line(f'TEST_top10_sol_same_{name}.txt', str(conduction_constant))
    if signals:
        append_new_line(f'TEST_top10_sol_same_{name}.txt', str(signals_true))
    append_new_line(f'TEST_top10_sol_same_{name}.txt', "=====================================================")
    append_new_line(f'TEST_top10_sol_alt_{name}.txt', "=====================================================")
    append_new_line(f'TEST_top10_sol_alt_{name}.txt', str(y_true[:(n_Rwaves-1)]))
...
def get_outputs_signals_recurrent_matching(true_stats):
    with torch.no_grad():
        ...
        Model.load("model_signals_rcINN_matching.pth", optim_rcINN, model_rcINN)
        Model.load("model_signals_sequence.pth", optim_seq, model_seq)
        Model.load("model_signals_matching.pth", optim_matching, model_matching)
        model_rcINN.eval()
        model_seq.eval()
        model_matching.eval()
        ...
        for k in range(len(true_stats)):
            y_matching = np.stack([(true_stats[k][0] - y_mean) / y_std]*5000, axis=0)
            ...
            output_matching = model_matching(z_matching, c=y_matching, rev=True)
        ...
        print("done with multiprocessing matchings")
        ...
        for k in range(len(paired_totals)):
            ...
            for i in range(len(paired_totals[k])):
                ...
                for j in range(len(paired_totals[k][i][1])):
                    z_stack.append(np.random.randn(50, 5, paired_totals[k][i][0]))
                    cond_stack.append(np.stack([datagen.y_to_cond(paired_totals[k][i][1][j], paired_totals[k][i][0], true_stats[k][0])]*50, axis=0))
                    counter += 1
                    if counter == 10:
                        break
            ...
            for i in range(len(y_rcINN)):
                ...
                y_rcINN[i][:, 0, :] = (y_rcINN[i][:, 0, :] - cond_mean[0]) / cond_std[0]
                y_rcINN[i][:, 1, :] = (y_rcINN[i][:, 1, :] - cond_mean[1]) / cond_std[1]
                y_rcINN[i] = torch.tensor(y_rcINN[i], dtype=torch.float32)
            ...
            for i in range(len(y_rcINN)):
                ...
                output_rcINN = model_rcINN(z_rcINN[i], c=y_rcINN[i], rev=True, recurrent=True)
                output_rcINN = output_rcINN.cpu().detach()
                
                output_rcINN[:, -2, :] = output_rcINN[:, -2, :] * aa_std + aa_mean
                output_rcINN[:, -1, :] = output_rcINN[:, -1, :] * cc_std + cc_mean
...
def process_outputs_signals_recurrent_matching(big_output, true_stats):
    ...
    for i in range(len(big_output)):
        for j in range(len(big_output[i])):
            ...
            for res_i in res:
                if len(res_i) != 0:
                    filtered_bp.append([res_i[0], res_i[1], [float(aa_ij), float(cc_ij)], np.array([lvl1, lvl2, lvl3], dtype='int16')])
    ...
    with torch.no_grad():
        ...
        for j in range(len(final_lengths)):
            ...
            for seq_len in final_lengths[j]:
                y_array = np.stack([(true_stats[j][0] - y_mean) / y_std]*seq_len, axis=1)
                ...
                output = model_rcINN(z_array, c=y_array, rev=True, recurrent=True)
...
def process_outputs_signals(big_output, true_stats):
    ...
    for i in range(len(big_output)):
        ...
        for res_i in res:
            if len(res_i) != 0:
                filtered_bp.append([res_i[0], res_i[1], [float(aa[i]), float(cc[i])], np.array([lvl1, lvl2, lvl3], dtype='int16')])
...
def process_outputs_splitter(data):
    ...
    for i in range(len(output_bp)):
        bp, bp_type = datagen.check_block_pattern_splitter(output_bp[i], n_Rwaves, splitter_type)
        if len(bp) != 0:
...
def process_outputs_bp_recurrent(outputs, true_stats):
    ...
    for i in range(len(outputs)):
        for j in range(len(outputs[i])):
            ...
            bp_i_checked, bp_type = datagen.check_block_pattern_alt(bp_i, n_Rwaves)
...
def get_outputs_bp_recurrent_matching(true_stats):
    with torch.no_grad():
        aa_mean = np.loadtxt("aa_mean_est.csv")
        aa_std = np.loadtxt("aa_std_est.csv")
        cc_mean = np.loadtxt("cc_mean_est.csv")
        ...
        for k in range(len(paired_totals)):
            ...
            for i in range(len(paired_totals[k])):
                ...
                counter = 0
                for j in range(len(paired_totals[k][i][1])):
                    z_stack.append(np.random.randn(50, 26, paired_totals[k][i][0]))
                    cond_stack.append(np.stack([datagen.y_to_cond(paired_totals[k][i][1][j], paired_totals[k][i][0], true_stats[k][0])]*50, axis=0))
                    counter += 1
                    if counter == 10:
                        break
