...
def subnet_fc(dims_in, dims_out):
    return nn.Sequential(nn.Linear(dims_in, config.n_hidden_layer_size), 
                         ...
                         nn.ELU(),
                         nn.Linear(config.n_hidden_layer_size,  config.n_hidden_layer_size), 
                         nn.BatchNorm1d(config.n_hidden_layer_size), 
                         nn.ELU(),
...
class subnet(nn.Module):
    def __init__(self, dims_in, dims_out):
        ...
        self.lstm = nn.LSTM(dims_in, config.hidden_size, config.rnn_layers, batch_first=True, bidirectional=True)
...
def generate_cINN_old():
    ...
    for k in range(config.n_blocks):
        nodes.append(Ff.Node(nodes[-1],
                             ...
                             {'subnet_constructor':subnet_fc, 'clamp':config.clamp},
                             conditions=cond,
                             name=F'coupling_{k}'))
        nodes.append(Ff.Node(nodes[-1],
                             Fm.PermuteRandom,
...
def generate_rcINN_old():
    ...
    for k in range(config.n_blocks):
        nodes.append(Ff.Node(nodes[-1],
                             Fm.GLOWCouplingBlock,
                             {'subnet_constructor':subnet_func, 'clamp':config.clamp},
                             conditions=cond,
                             name=F'coupling_{k}'))
        nodes.append(Ff.Node(nodes[-1],
                             Fm.PermuteRandom,
